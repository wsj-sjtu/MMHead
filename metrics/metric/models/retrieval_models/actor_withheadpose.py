...
class ACTORStyleEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.nfeats = nfeats
        self.projection = nn.Linear(nfeats, latent_dim -56)
        
        self.headpose_projection = nn.Linear(3, 56)
    ...
    def forward(self, x_dict: Dict) -> Tensor:
        x = x_dict["x"]
        mask = x_dict["mask"]
        headpose = x_dict['headpose']

        x = self.projection(x)
        x_headpose = self.headpose_projection(headpose)
        
        x = torch.cat((x, x_headpose),  dim = -1)
...
class ACTORStyleEncoder_text(nn.Module):
    # Similar to ACTOR but "action agnostic" and more general
    def __init__(
        self,
        nfeats: int,
...
class ACTORStyleDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.final_layer_headpose = nn.Linear(latent_dim, 3)
    ...
    def forward(self, z_dict: Dict) -> Tensor:
        ...
        output_med = self.seqTransDecoder(
            tgt=time_queries, memory=z, tgt_key_padding_mask=~mask
        )

        output = self.final_layer(output_med)
        
        output_headpose = self.final_layer_headpose(output_med)
        # zero for padded area
        output[~mask] = 0
        output_headpose[~mask] = 0
        return output, output_headpose
