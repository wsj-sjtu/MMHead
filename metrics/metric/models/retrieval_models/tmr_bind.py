...
from models.utils.losses import InfoNCE_with_filtering, KLLoss
...
class tmr(nn.Module):
    def __init__(
        ...
        self.fact = 1
        self.lmd = {"recons": 1.0, "latent": 1.0e-5, "kl": 1.0e-5, "contrastive": 0.1}
    ...
    def compute_loss(self, motion_x_dict, text_x_dict, return_all=False):
        ...
        # text -> motion
        t_motions, t_latents, t_dists = self.forward_text(text_x_dict, mask=mask)

        # motion -> motion
        m_motions, m_latents, m_dists = self.forward_motion(motion_x_dict, mask=mask)
